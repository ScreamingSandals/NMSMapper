    ...
    public MethodSpec generateSymbolAccessor(Accessor accessor, AccessorClassGenerator generator) {
        ...
        var optional = accessor.getMapping().node("fields")
                ...
                .filter(n -> n.node(getMapping().toUpperCase()).childrenMap().entrySet().stream().anyMatch(n1 ->
                        n1.getValue().getString("").equals(getName()) && (getForcedVersion() == null || Arrays.asList(n1.getKey().toString().split(",")).contains(getForcedVersion()))
                                && (
        ...
        if (optional.isPresent()) {
            ...
            return MethodSpec.methodBuilder("getField" + capitalized + (count != 1 ? count : ""))
                    ...
                    .addStatement("return $T.$N($T.class, $S, $L)", generator.getAccessorUtils(), "getEnumField", ClassName.get(generator.getBasePackage(), accessor.getClassName()), getName() + count, generator.generateMappings(n))
                    .build();
        } else {
